...
func outputMessage(message interface{}, messageType string, args ...interface{}) {
	...
	if OutputConsole {
		switch messageCast := message.(type) {
		case int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64:
			log.Printf("[%s] [%s] :: %d", files, messageType, messageCast)
			break
		default:
			if len(args) > 0 {
				log.Printf("[%s] [%s] :: %s", files, messageType, fmt.Sprintf(message.(string), args...))
			} else {
				log.Printf("[%s] [%s] :: %+v", files, messageType, message)
			}
			break
		}
	}
